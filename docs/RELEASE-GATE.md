...
>
> **状态区分（禁止混用）：**
> - `自动化通过`：CI / 本地脚本可验证的检查项已通过
> - `历史实机通过`：在历史版本上实测通过，不能作为当前版本放行证据
> - `当前候选实机通过`：当前候选版本上实测通过，构成放行证据
> - `未执行`：尚未测试
> - `失败`：已测但不通过，阻断发布
>
> 当前版本：v1.0.8（commit `be0cad9bf97b802b4aeb7158bf35f96281cee350`；tag 已校正）。
> `main` 分支包含 v1.0.8 之后的 CI 修复（`b5b61ff`），将进入未来版本，不复用 v1.0.8。
> **旧版本（≤ v1.0.7）的实机结果不作为 v1.0.8+ 的放行证据**，以下历史记录仅作对照。

## 当前已知阻断项

- [x] **设置页从文件导入 YAML 的 fs 权限**：已确认修复（前端统一走后端 Rust command）。
- [x] **v1.0.8 tag 可追溯性**：tag 曾被移动至 `b5b61ff`（CI 修复 commit），已于 2026-08-29 校正回 `be0cad9`（正式 Release commit）。Release `target_commitish` 始终为 `be0cad9`，未受影响。
- [ ] **GitHub Repository Ruleset（Immutable release tags）**：需仓库管理员手动创建（见 `docs/RELEASE-TAG-PROTECTION.md`）。仓库设置待人工完成。
